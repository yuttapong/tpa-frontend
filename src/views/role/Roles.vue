            ...
            <div class="tab-content pt-2">
              <div class="tab-pane fade show active qt-role" id="qt-role">
                <form @submit.prevent="onSearch()">
                  <div class="d-flex flex-wrap gap-2">
                    <div>
                      <BButton variant="primary" @click="addRole($event)" size="sm"><i class="bi bi-plus"></i></BButton>
                    </div>
                    <div class="">
                ...
                <table class="table table-sm">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in roles" :key="index">
                      <th scope="row">
                        <BButton variant="outline-secondary" @click="editRole(item, index, $event)" size="sm"><i
                            class="bi bi-pencil"></i></BButton>
                      </th>
                      <th scope="row">{{ item.id }}</th>
                      <td>
                        <a role="button" @click="editRole(item, index, $event)"><i class="bi bi-person me-2"></i>{{
                          item.name }}</a>
                      </td>
                      <td>
                        {{ item.display_name }}
              ...
              <div class="tab-pane fade qt-permission" id="qt-permission">
                <form @submit.prevent="loadPermissions()">
                  <div class="d-flex flex-wrap gap-2">
                    <div>
                      <BButton @click="addPermission" variant="primary" size="sm"><i class="bi bi-plus"></i></BButton>
                    </div>
                    <div class="">
                      <input type="search" v-model="formSearch.q" class="form-control form-control-sm" placeholder=""
                  ...
                  <table class="table table-sm table-hover table-bordered">
                    ...
                    <tbody>
                      <tr v-for="(item, index) in permissions" :key="index">
                        <th scope="row">
                          <BButton variant="outline-secondary" @click="editPermission(item, index, $event)" size="sm"><i
                              class="bi bi-pencil"></i></BButton>
                        </th>
                        ...
                        <td>
                          <a role="button" @click="editPermission(item, index, $event)"><i class="bi bi-lock me-2"></i>{{
                            item.name }}</a>
                        </td>
                        <td>
                          {{ item.display_name }}
                        ...
                        <td>
                          <span class="badge rounded-pill bg-secondary text-light mx-1 fw-normal"
                            v-for="t in permisstionType" :key="t">{{ t.text }}</span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
...
Sunt est soluta temporibus accusantium neque nam maiores cumque temporibus. Tempora libero non est unde veniam est qui dolor. Ut sunt iure rerum quae quisquam autem eveniet perspiciatis odit. Fuga sequi sed ea saepe at unde.</textarea>
  ...
  <RoleForm :mode="formMode" v-model:visible="visibleModalRole" :data="viewRoleData" @onUpdated="onUpdatedRole"
    @onCreated="onCreatedRole" />
  <PermissionForm v-model:visible="visibleModalPermission" v-model:data="viewPermissionData"
    @onUpdated="onUpdatedPermission" @onCreated="onCreatedPermission" />
</template>
