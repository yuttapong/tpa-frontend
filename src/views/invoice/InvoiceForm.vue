  ...
  if (billItems.length > 0) {
    billItems.map((item) => {
      ...
      if (customerDiscount.value) {
        if (customerDiscount.value.discount_type == 'percent') {
          row.discount_customer_percent = Number(customerDiscount.value.discount)
        } else if (customerDiscount.value.discount_type == 'amount') {
          row.discount_customer = Number(customerDiscount.value.discount)
        }
      }
...
const fillDataCustomer = async (bill, billItems) => {
  ...
  if (bill) {
    ...
    getCustomerStatistics(data.customer_id)
...
const calculate = () => {
  let items = formInvoice.value.items.map((item) => {
    ...
    let discount = 0
    console.log(item)
    if (item.discount_customer_percent) {
      item.discount_customer = total * Number(item.discount_customer_percent) / 100;
    }
    if (item.discount_lab_percent) {
      item.discount_lab = total * Number(item.discount_lab_percent) / 100;
    }
    if (item.discount_order_percent) {
      item.discount_order = total * Number(item.discount_order_percent) / 100;
    }
    discount =
...
const onSelectCustomer = (data) => {
  ...
  formInvoice.value.address = removeUndefinedAndNull(address)
  getCustomerStatistics(formInvoice.value.customer_id)
}
...
const getInvoice = async (id) => {
  ...
  if (data) {
    ...
    let address = removeUndefinedAndNull(data.address)
    formInvoice.value.address = address
    if (!formInvoice.value.subdistrict)
      formInvoice.value.subdistrict = removeUndefinedAndNull(data.customer?.subdistrict)

    if (!formInvoice.value.district)
      formInvoice.value.district = removeUndefinedAndNull(data.customer?.district)

    if (!formInvoice.value.province)
      formInvoice.value.province = removeUndefinedAndNull(data.customer?.province)

    if (!formInvoice.value.postalcode)
      formInvoice.value.postalcode = removeUndefinedAndNull(data.customer?.postalcode)

    if (!formInvoice.value.country)
      formInvoice.value.country = removeUndefinedAndNull(data.customer?.country)
  }
  loading.value = false
  getCustomerStatistics(formInvoice.value.customer_id)
}
...
const tableFields = [
  ...
  { key: 'item_code', label: 'ItemCode', sortable: false },
          ...
          <div class="card m-0">
            <div class="card-body p-2">
              <form @submit.prevent="onSearch()">
                  ...
                  <div class="row g-2">
                    ...
                    <div class="col-6 col-md-4 col-lg-3" v-if="formInvoice.code">
                      ...
                      <input type="text" v-model="formInvoice.code" class="form-control form-control-sm"
                        placeholder="เลขที่ใบแจ้งหนี้" disabled="disabled" />
                    </div>
                ...
                <DiscountAndReword v-model:customer-id="formInvoice.customer_id" v-model:reward="customerReward"
                  v-model:discount="customerDiscount" class="my-1" />
                <section v-if="formMode == 'edit'" class="mt-1">
    ...
    " />
                        ...
                        <div class="input-group input-group-sm">
                          <div>
                            <BInput type="number" min="0" v-model="row.item.discount_lab_percent" style="width: 5rem"
                              ...
                              @blur="onChangeDiscountTypeOfItems(row.item, 'lab', 'percentage')" />
                          </div>
                          <div>
                            <BButton variant="text">%</BButton>
                          </div>
                        ...
                        <div class="input-group input-group-sm">
                          <div>
                            <BInput type="number" min="0" v-model="row.item.discount_order_percent" style="width: 5rem"
                              ...
                              @blur="onChangeDiscountTypeOfItems(row.item, 'order', 'percentage')" />
                          </div>
                          <div>
                            <BButton variant="text">%</BButton>
                          </div>
        ...
        <div class="col-12 col-md-12">
          ...
          <div class="d-flex flex-wrap gap-2 mt-3">
            <router-link to="/invoices" class="btn btn-outline-secondary btn-md mx-1" type="button">
            ...
            <button class="btn btn-primary btn-md mx-1" type="submit" @click="saveAndSend()" v-if="formMode == 'edit'">
