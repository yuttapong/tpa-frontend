...
const totalPages = computed(() => Math.ceil(Number(props.total) / Number(props.perPage)))
...
const showLastPageBreakpoint = computed(
...
const showLastBreakpoint = computed(
  () => paginate.value.pages[paginate.value.pages.length - 1] < totalPages.value - 2,
)
...
<template>
  paginate {{ props }} / {{ totalPages }}
  <nav aria-label="..." :class="attrs.class">
    <ul :class="bindPaginationClass">
      <li v-if="showJumpButtons" class="page-item" :class="{ disabled: firstPage }">
        <a class="page-link" href="#" @click.prevent="onChangePage(1)">{{ firstJumpContent }}</a>
      </li>
      <li
      ...
      <!-- ... -->
      <li v-if="showFirstPageBreakpoint" class="page-item" @click.prevent="onChangePage(1)">
      ...
      <li v-if="showLastPageBreakpoint" class="page-item" @click.prevent="onChangePage(totalPages)">
      ...
      <li v-if="showJumpButtons" class="page-item" :class="{ disabled: lastPage }">
        <a class="page-link" href="#" @click.prevent="onChangePage(totalPages)">{{
          lastJumpContent
        }}</a>
      </li>
    </ul>
