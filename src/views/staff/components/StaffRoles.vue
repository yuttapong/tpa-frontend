<template>
    <div class="row">
        <div class="col-12">
            ...
            <select v-model="roleSelected" class="form-select" @click="onSelected($event)" aria-label="เลือกห้องแล็ป"
                multiple style="min-height: 300px;">
                <optgroup label="Roles">
                    <option v-for="(item, key) in filterRoles" :value="item">{{ `${item?.name} - ${item?.description}`
                    }}
                    </option>
                </optgroup>

                <optgroup label="Permissions">
                    <option v-for="(item, key) in filterPermissions" :value="item">{{ `${item?.name} - ${item?.description}`
                    }}
                    </option>
                </optgroup>
            </select>
