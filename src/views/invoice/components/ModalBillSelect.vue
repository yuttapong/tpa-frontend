                        ...
                        <form @submit.prevent="search()">
                            <div class="d-flex gap-2">
                                <div class="discountLab">
                                    <input type="search" v-model="formSearchProduct.bill_code"
                                        class="form-control form-control-sm" placeholder="เลขที่ใบขอรับบริการ"
                                        @keyup.enter="search()" />
                                </div>
                                <div class="discountLab">
                                    <input type="search" v-model="formSearchProduct.item_code"
                                        class="form-control form-control-sm" placeholder="เลขที่ WorderOrder"
                                        @keyup.enter="search()" />
                                </div>
                                <div class="discountLab">
                                    <input type="search" v-model="formSearchProduct.customer_id"
                        ...
                        <table class="table table-sm table-striped table-bordered">
                            <thead>
                                <tr>
                                    ...
                                    <!-- <th scope="col">#</th> -->
                                    <th scope="col" class="fw-bold">Bill ID</th>
                                ...
                                <tr v-for="(item, index) in items" :key="index">
                                    <th scope="row">
                                        <template v-if="!existCarts(item)">
                                            <button class="btn btn-secondary btn-sm d-block" type="button"
                                    ...
                                    <td>
                                        <span class="badge bg-secondary text-light p-2">{{ item.code }}</span>
                                    </td>
                                    <td>
                                        <div class="">
                                    ...
                                    <td>{{ parseFloat(item.grand_total).toLocaleString() }}</td>
